...
Teisoro use: the drawer adjustment dialog's direction (`docs/design/drawer-close/08-adjustment-dialog.md`); later the vault movement direction (S19).
Proposed API: `variant?: 'compact' | 'filled'` on `SegmentedControl`, default `compact` (unchanged look). `filled` adds `sw-segmented-filled`: the track stretches to its container (`display: flex; width: 100%`), each item takes an equal share (`flex: 1 1 0`, centered, `md` control height, body type), and the selected item paints `--sw-color-accent` with `--sw-color-onAccent`.
