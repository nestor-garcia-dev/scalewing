...
Native `TabBar` and `Table` are in this checkout for FutMas
(`docs/requests/futmas-tab-bar.md`, `docs/requests/futmas-table.md`).
Consumers verify with local `link:`. Do not wait for npm. `v0.3.0` remains the
published pin until the next release.
...
- FutMas consumes `@scalewing/react-native@0.3.0` from the Dragon Tails sibling checkout (`/Users/Neto/Projects/Dragon-Tails/scalewing`). New primitives are implemented here, then verified with FutMas `pnpm scalewing:link`. Named-palette choice for FutMas remains open; do not invent a second skin in the app.
